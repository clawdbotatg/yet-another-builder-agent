...
#
# Usage: ./steps/09_frontend.sh <model> <project-name>
#
# Must run AFTER Step 8 (deploy) so deployedContracts.ts already contains
# real deployed addresses. The LLM receives deployedContracts.ts as context
# and must NOT hardcode addresses or pass them as props.
#
# Inputs:
#   <model>         LLM model ID
#   <project-name>  From params.json
#   Reads: job.md, params.json, deployed contract ABI, deployedContracts.ts,
#          externalContracts.ts, existing SE2 scaffold
#
# Outputs:
...
MODEL="${1:?Usage: ./steps/09_frontend.sh <model> <project-name>}"
PROJECT="${2:?Usage: ./steps/09_frontend.sh <model> <project-name>}"
...
# ─── Step 9a: Update scaffold.config.ts (deterministic) ─────────────────────
...
# ─── Step 9b: Gather context for LLM ────────────────────────────────────────
...
# Read deployedContracts.ts — generated by Step 8, contains actual deployed addresses
DEPLOYED_CONTRACTS_TS=""
DEPLOYED_TS_PATH="$NEXTJS_DIR/contracts/deployedContracts.ts"
if [[ -f "$DEPLOYED_TS_PATH" ]]; then
  DEPLOYED_CONTRACTS_TS=$(cat "$DEPLOYED_TS_PATH")
  echo "  ✓ deployedContracts.ts found (will be passed as context)"
else
  echo "  ⚠ deployedContracts.ts not found — run Step 8 (deploy) first"
  echo "    Continuing without it, but address resolution may be incorrect"
fi
...
# ─── Step 9c: LLM call to generate all frontend files ───────────────────────
echo ""
echo "  Generating frontend files with LLM..."

SYSTEM_PROMPT='You are a frontend developer building a Scaffold-ETH 2 dApp.

You will be given a build plan with a design brief, contract source code, deployedContracts.ts (with real deployed addresses), and existing SE2 scaffold files. Generate the complete frontend.
...
// Get a deployed contract address inside a component (use this — never hardcode):
...
CRITICAL ADDRESS RULES (the most common bug class — never violate):
- NEVER hardcode a deployed contract address as a string literal in any React file
- NEVER pass a deployed contract address as a prop from page.tsx into a child component
  BAD:  const ADDR = "0x808b..." as `0x${string}`; <PostForm contractAddress={ADDR} />
  GOOD: Inside PostForm, call useDeployedContractInfo("BurnBoard") to get the address
- For ANY operation that needs a deployed contract address (e.g. as spender in ERC20 approve):
  use useDeployedContractInfo("ContractName") INSIDE the component that needs it
- The deployedContracts.ts file is provided in context — it shows what is deployed
  Use it for REFERENCE only. Do NOT copy its addresses into component files as literals.
- External contracts (tokens, protocols) are accessed via contractName in hooks — never hardcode
- page.tsx should compose components, NOT know any contract addresses
...
- Only generate components that are actually imported and used — no dead code
...
## Deployed Contracts (auto-generated by forge deploy — use via useDeployedContractInfo hooks, NOT as hardcoded literals)
$DEPLOYED_CONTRACTS_TS
